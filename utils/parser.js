...
function parseNotificationText(text) {
  ...
  let bankName = "CASH WALLET"; // डिफ़ॉल्ट फॉलबैक
  let bankBalance = null; 

  if (!text || typeof text !== "string") {
    return { amount, merchant, senderName, receiverName, bankName, bankBalance };
  }

  const cleanText = text.replace(/\s+/g, " ").trim();
  const lowerText = cleanText.toLowerCase();

  // 🏦 1. STRICT BANK DETECTION
  if (lowerText.includes("cboi") || lowerText.includes("central bank")) {
    bankName = "CENTRAL BANK OF INDIA";
  } else if (lowerText.includes("axis")) {
    bankName = "AXIS BANK";
  } else if (lowerText.includes("hdfc")) {
    bankName = "HDFC BANK";
  } else if (lowerText.includes("sbi") || lowerText.includes("sbin")) {
    bankName = "STATE BANK OF INDIA";
  } else if (lowerText.includes("icici")) {
    bankName = "ICICI BANK";
  }

  // 💰 2. TRANSACTION AMOUNT EXTRACTION
  const amountRegex = /(?:INR|Rs\.?|₹|debited|credited)\s*([\d,]+\.?\d*)/i;
  let amountMatch = cleanText.match(amountRegex);
  ...
  // 📊 3. LIVE BANK BALANCE EXTRACTION (मैसेज से डायरेक्ट कुल बैलेंस निकालना)
  const balanceRegex = /(?:Total\s+Bal|Clr\s+Bal|Balance|Avl\s+Bal|Bal)\s*(?:INR|Rs\.?|₹)?\s*([\d,]+\.?\d*)/i;
  const balanceMatch = cleanText.match(balanceRegex);
  if (balanceMatch && balanceMatch[1]) {
    bankBalance = parseFloat(balanceMatch[1].replace(/,/g, ""));
  }
  ...
  if (upiMatch && upiMatch[1]) {
    ...
    if (isDebit) {
      senderName = "My Account";
      const receiverPatterns = [/(?:paid to|transferred to|spent on|at|to)\s+([^.]+)/i, /vpa\s+([^.]+)/i];
      for (const pattern of receiverPatterns) {
        const match = cleanText.match(pattern);
        if (match && match[1]) { receiverName = match[1].trim(); break; }
      }
      merchant = receiverName !== "Unknown Receiver" ? receiverName : "Other Merchant";
    } else {
      receiverName = "My Account";
      const senderPatterns = [/(?:by|received from|deposited by|transfer from)\s+([^.]+)/i, /from\s+([^.]+)/i];
      for (const pattern of senderPatterns) {
        const match = cleanText.match(pattern);
        if (match && match[1]) { senderName = match[1].trim(); break; }
      }
      merchant = senderName !== "Unknown Sender" ? senderName : "Inward Remittance";
    }
  }

  // 🛑 5. CLEANING GATEKEEPER
  const cleanFinalName = (name) => {
    let n = name.split("\n")[0].trim();
    n = n.replace(/(not you|sms block|a\/c no|ref no|total bal|clr bal|\d{10,}).*/i, "");
    n = n.replace(/[\s\-\.,\/]+$/, "").trim();
    return n;
  };
  ...
  return {
    ...
    receiverName,
    bankName,
    bankBalance: isNaN(bankBalance) ? null : bankBalance
  };
}
